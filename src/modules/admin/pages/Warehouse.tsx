...
import type { WarehouseItem } from "@/lib/network/types/warehouse.types";
import type {
  ItemCategory,
  StockMovementType,
...
const MovementsList = ({ itemId }: { itemId: string }) => {
  const { data, isLoading } = useGetWarehouseMovements(itemId);
...
export default function Warehouse() {
  ...
  const [itemModal, setItemModal] = useState<null | { item?: WarehouseItem }>(
    null,
  );
  const [adjustFor, setAdjustFor] = useState<WarehouseItem | null>(null);
  const [movementsFor, setMovementsFor] = useState<WarehouseItem | null>(null);
  ...
  const { data, isLoading } = useGetWarehouseItems({
    page,
    pageSize,
  ...
  return (
    <>
      <PageMeta title="Warehouse | KGR Console" />
      <PageHead
        eyebrow="OFF SITE"
        title="Warehouse"
        subtitle="Stock held at the warehouse, separate from yard inventory, with every movement on record."
        actions={
          canStock ? (
            <button
              type="button"
          ...
          <div className="flex flex-col items-center gap-3 px-6 py-14 text-center">
            ...
            <p className="m-0 text-[15px] font-bold text-bark">
              {search || category !== "all" || lowStock
                ? "Nothing in the warehouse matches this view."
                : "The warehouse is empty. Add the first item."}
            </p>
          </div>
        )}
        ...
        {adjustFor && (
          <div className="flex flex-col gap-4">
            <p className="m-0 text-[13px] font-semibold text-fog">
              Currently {adjustFor.quantityOnHand} {adjustFor.unit} in the
              warehouse.
            </p>
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="wh-adj-qty" className={labelClasses}>
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="wh-adj-note" className={labelClasses}>
                Note
              </label>
              <input
                id="wh-adj-note"
